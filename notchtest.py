...
def notchtest(a, monitorheight, resolution):
	...
	def writeme(value, phasecompensation, ii):
		# wmr = list(np.real(np.divide(value, phasecompensation)))		# 7
		# wmi = list(np.imag(np.divide(value, phasecompensation)))		# 8
		wmr = list(np.abs(np.divide(value, phasecompensation)))		# 7
		wmi = list(np.angle(np.divide(value, phasecompensation)))		# 8
		f2.write("%s, " % wmr[ii])		# 7
		f2.write("%s, " % wmi[ii])		# 8
